...
#define max_AmpMoyenne 20                                                  //Cette variable sera utilisée pour définir la valeur de réference pour le remplissage du bargraph, exemple : max max_AmpMoyenne est mis à 200 et que la variable à tester atteint la valeur de 100 alors le bargraph fera 50% de la taille de l'écran.
#define TIMER1_TOP (340)                                                    // (343) 17Hz par unité, augmenter la valeur réduit la fréquence de sortie
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
long ValeurBrut[4];                                                      //Les "ValeurBrut[]"" sont utilisées pour accumuler la lecture des données du CAN, une pour chacune des 4 phases.
uint16_t Echantillon = 0;                                                   //variable utilisée pour gérer les séquences d'échantillonnage.
const uint16_t EchantillonageMax = 1023;                                    //Constante qui sert à définir un plafond pour la variable "Echantillon", ici 1024 (10bits) représente le nombre de bits du CAN.
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
volatile long Moyenne[4];                                                //Quand nous avons accumulé suffisamment de lectures dans les "ValeursBrutes[]", l'ISR les copie dans cette variable et recommence.
...
long calib[4];                                                           //Valeur (réglée pendant la calibration) que l'on extrait de la variable "Moyenne" et qui servira pour recalibrer le détecteur.
...
const double f = 80.0;                                                     //(200) Variables utilisées pour le filtrage et le traitement numérique des données.
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
float Seuil = 1;                                                         //Variable pour contrôler la sensibilité de la détection, une valeur faible correspond à une plus grande sensibilité. Par exemple, 10 est utilisable uniquement si nous avons une bobine parfaitement équilibrée.
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
/*----------------------INITIALISATION DES PINS, DU PORT SERIE ET DE L'ECRAN LCD--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
  ...
  arduinoNano.alimentation_init();    
                                        //Appel de la fonction de contrôle de l'alimentation.
...
ISR(TIMER1_OVF_vect)                                                        //Interruption de débordement du Timer1.
{
  ...
  int16_t val = (int16_t)ADCH;                                    //Nous avons uniquement besoin de lire les 8 bits les plus significatifs.
  ...
  long *p = &ValeurBrut[compteur & 3];                                   //Crée un pointeur 'p' qui pointe vers 'ValeurBrut'. Le masque '&' (ET binaire) permet de limiter le nombre d'opérations jusqu'à ce que 'compteur' atteigne 3.
  if (compteur < 4)                                                         //Exécution du code tant que 'compteur' ne dépasse pas 4.                            
  {
    *p += (val);                                                            //Incrémente la valeur stockée à l'adresse du pointeur par la valeur stockée dans 'val'.   
    if (*p > 200000) *p = 200000;                                             //Plafonne la valeur maximale stockée à l'adresse du pointeur à 15 000.
  }
  else    //éxecution du code lors ce que compteur dépasse 45               //Exécution du code si 'compteur' dépasse 4.  
  {
    *p -= val;                                                              //Décrémente la valeur stockée à l'adresse du pointeur par la valeur stockée dans 'val'.
    if (*p < -200000) *p = -200000;                                           //Plafonne la valeur minimale stockée à l'adresse du pointeur à - 15 000.
  } 
...
/*----------------------APPEL DE LA FONCTION POUR LE CONTROLE DE L'ALIMENTATION--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
  //arduinoNano.stop_alimentation();                                       //La valeur entre parenthèses représente le temps en secondes nécessaire pour l'appui sur le bouton d'alimentation afin de couper l'alimentation.
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
  sei();
...
/*----------------------TEST DE CONDITION POUR DETERMINER LA NATURE DE L'OBJET DETECTE--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    if (AmpMoyenne < Seuil)
    {
        Serial.println("no detec");                                        //Affichage du type de métal découvert sur l'écran LCD et le port série en fonction de la valeur de 'phaseMoyenne'.   
        lcd.setCursor(0,0);       
        lcd.print("no detec");
        Serial.println ();
        Serial.println("---------------------------------------------------------------------------");
